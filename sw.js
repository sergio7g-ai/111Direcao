const CACHE_NAME = "orienta-te-direcao-v3";
...
self.addEventListener("fetch", event => {
  ...
  if(url.origin === location.origin && (event.request.mode === "navigate" || url.pathname.endsWith("/index.html") || url.pathname.endsWith("/"))){
  ...
  event.respondWith(caches.match(event.request).then(cached => cached || fetch(event.request).then(response => { const copy=response.clone(); if(url.origin===location.origin) caches.open(CACHE_NAME).then(c=>c.put(event.request,copy)); return response; }).catch(()=>cached)));
});
